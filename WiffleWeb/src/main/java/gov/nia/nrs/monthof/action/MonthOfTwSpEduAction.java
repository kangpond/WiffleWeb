 ...
 * @author Tim
 */
@Namespace("/monthOfTwSpEdu")
public class MonthOfTwSpEduAction extends MonthOfAction
{
	private static final long serialVersionUID = 1237965944178758555L;

	private static final Logger logger = Logger.getLogger(MonthOfTwSpEduAction.class);
	...
	@Action(value = "query", results = { @Result(name = SUCCESS, location = "monthOfTwSpEduQuery", type = "tiles") })
	...
	@Action(value = "list", results = { @Result(name = SUCCESS, location = "monthOfTwSpEduList", type = "tiles"),
			@Result(name = INPUT, location = "monthOfTwSpEduQuery", type = "tiles") })
	...
	{
		String fileName = getFileName("教育程度每月分析表");

		TreeMap<Integer, QueryAttrEnum> rowAttrMap = new TreeMap<Integer, QueryAttrEnum>();
		rowAttrMap.put(0, QueryAttrEnum.SpouseOccEduAccu_Education);
	...
	public String getWhereFilter()
	{
		...
		if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
			whereFilter.append(MultiQueryUtils.getWhereFilter(QueryAttrEnum.City, cityResult));
		return whereFilter.toString();
	}

	@Action(value = "report", results = {
			@Result(name = INPUT, location = "monthOfTwSpEduList", type = "tiles"),
