 ...
 * @author Tim
 */
@Namespace("/monthOfOcc")
public class MonthOfOccAction extends MonthOfAction
{
	private static final long serialVersionUID = -1870890425333576212L;

	private static final Logger logger = Logger.getLogger(MonthOfOccAction.class);
	...
	@Action(value = "query", results = { @Result(name = SUCCESS, location = "monthOfOccQuery", type = "tiles") })
	...
	@Action(value = "list", results = { @Result(name = SUCCESS, location = "monthOfOccList", type = "tiles"),
			@Result(name = INPUT, location = "monthOfOccQuery", type = "tiles") })
	...
	{
		String fileName = getFileName("職業每月分析表");

		TreeMap<Integer, QueryAttrEnum> rowAttrMap = new TreeMap<Integer, QueryAttrEnum>();
		rowAttrMap.put(0, QueryAttrEnum.SpouseOccEduAccu_Occupation);
	...
	@Action(value = "report", results = {
			@Result(name = INPUT, location = "monthOfOccList", type = "tiles"),
