 ...
 *
 */
@Namespace("/twSpouseAge")
public class TWSpouseAgeAccuAction extends MonthOfAction implements Preparable {

	private static final Logger logger = Logger.getLogger(TWSpouseAgeAccuAction.class);
	...
	@Action(value = "/twSpouseAge/query",results={
			@Result(name = "success", location = "twSpouseAge.query",type="tiles")
			})
	@SkipValidation
	public String query(){
		return SUCCESS;
	...
	@Action(value = "/twSpouseAge/list",results={
			@Result(name = SUCCESS, location = "twSpouseAge.result",type="tiles"),
			@Result(name = INPUT, location = "twSpouseAge.query",type="tiles")
			})
	@SkipValidation
	public String list(){
		try{
	...
	private MultiQueryForm getReportForm(){
		String fileName = "國人涉外婚姻趨勢統計分析表(台籍配偶年齡分佈狀況)";

		TreeMap<Integer, QueryAttrEnum> rowAttrMap = new TreeMap<Integer, QueryAttrEnum>();
		rowAttrMap.put(0, isBefore2016() ?QueryAttrEnum.TwSpAge2016:QueryAttrEnum.TwSpAge2017);
		...
		form.setTable(TableNameEnum.TWSpouseAgeAccu);
	...
	@Action(value = "report", results = {
			@Result(name = INPUT, location = "twSpouseAge.result",type="tiles"),
	...
	public String barChart_ajax() throws Exception {
		...
		try {
			...
			attrMap.put(1, isBefore2016() ?QueryAttrEnum.TwSpAge2016:QueryAttrEnum.TwSpAge2017);
	...
	public String pieChart_ajax() throws Exception {
		...
		try{
			TreeMap<Integer, QueryAttrEnum> attrMap = new TreeMap<Integer, QueryAttrEnum>();
			attrMap.put(0, isBefore2016() ?QueryAttrEnum.TwSpAge2016:QueryAttrEnum.TwSpAge2017);
