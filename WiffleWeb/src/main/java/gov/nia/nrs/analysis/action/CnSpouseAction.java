 ...
 */
@Namespace("/cnSpouse")
public class CnSpouseAction extends MonthOfAction implements Preparable{

	private static final Logger logger = Logger.getLogger(CnSpouseAction.class);

	
	private static final long serialVersionUID = 3848770972690110217L;
	...
	@Action(value = "/query",results={
			@Result(name = "success", location = "cnSpouse.birthPlace.search",type="tiles")
			})
	@SkipValidation
	public String query(){
		return SUCCESS;
	...
	@Action(value = "/list",results={
			@Result(name = SUCCESS, location = "cnSpouse.birthPlace.result",type="tiles"),
			@Result(name = INPUT, location = "cnSpouse.birthPlace.search",type="tiles")
			})
	@SkipValidation
	public String list(){
		try{
	...
	private MultiQueryForm getReportForm(){
		String fileName = "國人涉外婚姻趨勢統計分析表(大陸配偶各省份)";

		TreeMap<Integer, QueryAttrEnum> rowAttrMap = new TreeMap<Integer, QueryAttrEnum>();
		rowAttrMap.put(0, QueryAttrEnum.BirthPlace);
		...
		form.setTable(TableNameEnum.CnSpouseBirthPlaceAccu);
	...
	@Action(value = "report", results = {
			@Result(name = INPUT, location = "cnSpouse.birthPlace.result",type="tiles"),
	...
	public String barChart_ajax() throws Exception {
		...
		try {
			...
			attrMap.put(0, QueryAttrEnum.BirthPlace);
			attrMap.put(1, QueryAttrEnum.MarryStatus);
