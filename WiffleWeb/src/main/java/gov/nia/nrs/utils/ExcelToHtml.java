...
public class ExcelToHtml
{
	...
	/**
	 * (Each Excel sheet cell becomes an HTML table cell) Generates an HTML table cell which has the same font styles, alignments, colours and borders
	 * as the Excel cell.
	 *
...
//			mergeStart = -1;
//			mergeEnd = -1;
			mergeEnd += (mergeEnd - mergeStart + 1);//修正一列需要多次合併儲存格的問題
		...
		try
		{
			...
			case HSSFCell.CELL_TYPE_FORMULA:
				...
				case Cell.CELL_TYPE_NUMERIC:
					//Excel的數值內容是否顯示小數點
					out.append(String.valueOf((int) cv.getNumberValue()));
					break;
	...
	public static void main(String[] args)
	{
		try
		{
			File initialFile = new File("D:\\Project\\2015BA112_Cloud\\education.xls");
			InputStream targetStream = new FileInputStream(initialFile);
			ExcelToHtml eth = new ExcelToHtml(targetStream);
			System.out.println(eth.getHTML());
		}
		catch (Exception e)
		{
			e.printStackTrace();
