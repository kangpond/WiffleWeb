 ...
 */
@Namespace("/cityOfOcc")
public class CityOfOccAction extends CityOfAction
{
	private static final long serialVersionUID = -7730867850119755009L;

	private static final Logger logger = Logger.getLogger(CityOfOccAction.class);
	...
	@Action(value = "query", results = { @Result(name = SUCCESS, location = "cityOfOccQuery", type = "tiles") })
	...
	@Action(value = "list", results = { @Result(name = SUCCESS, location = "cityOfOccList", type = "tiles"),
			@Result(name = INPUT, location = "cityOfOccQuery", type = "tiles") })
	...
	{
		String fileName = getFileName("職業各縣市分布表");
		...
		colAttrMap.put(0, QueryAttrEnum.SpouseOccEduAccu_Occupation);
		...
		if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
		{
			...
			colAttrMap.put(0, QueryAttrEnum.Occupation);
			colAttrMap.put(1, QueryAttrEnum.Gender);
	...
	public String getWhereFilter()
	{
		...
		if (MultiQueryUtils.isEmptyOrUnSelect(cityResult))
		{
			whereFilter.append(" and CategoryType='1'");
		}
		return whereFilter.toString();
	}

	@Action(value = "report", results = {
			@Result(name = INPUT, location = "cityOfOccList", type = "tiles"),
	...
	public String barChart_ajax() throws Exception
	{
		...
		attrMap.put(1, QueryAttrEnum.SpouseOccEduAccu_Occupation);
		try
		{
			...
			if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
			{
				...
				attrMap.put(1, QueryAttrEnum.Occupation);
	...
	public String pieChart_ajax() throws Exception
	{
		...
		TreeMap<Integer, QueryAttrEnum> attrMap = new TreeMap<Integer, QueryAttrEnum>();
		attrMap.put(0, QueryAttrEnum.SpouseOccEduAccu_Occupation);
		try
		{
			MultiQueryForm form = getChartForm(attrMap);
			form.setTable(table);
			...
			{
				attrMap = new TreeMap<Integer, QueryAttrEnum>();
				attrMap.put(0, QueryAttrEnum.Occupation);
				form.setRowAttrMap(attrMap);
				Map<String, String> paramMap = new HashMap<String, String>();
				paramMap.put(QueryAttrEnum.CityA.getColumnName(), cityResult);
				form.setParamMap(paramMap);
