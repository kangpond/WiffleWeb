 ...
 * @author Tim
 */
@Namespace("/cityOfTwSpEdu")
public class CityOfTwSpEduAction extends CityOfAction
{
	private static final long serialVersionUID = -7040422909245214632L;

	private static final Logger logger = Logger.getLogger(CityOfTwSpEduAction.class);

	private static TableNameEnum table = TableNameEnum.SpouseOccEduAccu;
	...
	@Action(value = "query", results = { @Result(name = SUCCESS, location = "cityOfTwSpEduQuery", type = "tiles") })
	...
	@Action(value = "list", results = { @Result(name = SUCCESS, location = "cityOfTwSpEduList", type = "tiles"),
			@Result(name = INPUT, location = "cityOfTwSpEduQuery", type = "tiles") })
	...
	private MultiQueryForm getReportForm()
	{
		...
		if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
		{
			...
			colAttrMap.put(0, QueryAttrEnum.Education);
			colAttrMap.put(1, QueryAttrEnum.Gender);
	...
	public String getWhereFilter()
	{
		...
		if (MultiQueryUtils.isEmptyOrUnSelect(cityResult))
		{
			whereFilter.append(" and CategoryType='3'");
		}
		return whereFilter.toString();
	}

	@Action(value = "report", results = {
			@Result(name = INPUT, location = "cityOfTwSpEduList", type = "tiles"),
	...
	public String barChart_ajax() throws Exception
	{
		...
		attrMap.put(0, QueryAttrEnum.City);
		attrMap.put(1, QueryAttrEnum.SpouseOccEduAccu_Education);
		try
		{
			...
			if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
			{
				...
				attrMap.put(1, QueryAttrEnum.Education);
	...
	public String pieChart_ajax() throws Exception
	{
		...
		TreeMap<Integer, QueryAttrEnum> attrMap = new TreeMap<Integer, QueryAttrEnum>();
		attrMap.put(0, QueryAttrEnum.SpouseOccEduAccu_Education);
		try
		{
			MultiQueryForm form = getChartForm(attrMap);
			form.setTable(table);
			...
			{
				attrMap = new TreeMap<Integer, QueryAttrEnum>();
				attrMap.put(0, QueryAttrEnum.Education);
				form.setRowAttrMap(attrMap);
				Map<String, String> paramMap = new HashMap<String, String>();
				paramMap.put(QueryAttrEnum.CityA.getColumnName(), cityResult);
				form.setParamMap(paramMap);
