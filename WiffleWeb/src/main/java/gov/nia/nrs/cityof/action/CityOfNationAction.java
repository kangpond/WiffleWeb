 ...
 * @author Tim
 */
@Namespace("/cityOfNation")
public class CityOfNationAction extends CityOfAction
{
	private static final long serialVersionUID = 6150691993350644892L;

	private static final Logger logger = Logger.getLogger(CityOfNationAction.class);

	private static TableNameEnum table = TableNameEnum.ForeignSpouseNaCityAccu;
	...
	@Action(value = "query", results = { @Result(name = SUCCESS, location = "cityOfNationQuery", type = "tiles") })
	...
	@Action(value = "list", results = { @Result(name = SUCCESS, location = "cityOfNationList", type = "tiles"),
			@Result(name = INPUT, location = "cityOfNationQuery", type = "tiles") })
	...
	private MultiQueryForm getReportForm()
	{
		...
		if (!MultiQueryUtils.isEmptyOrUnSelect(cityResult))
		{
			...
			colAttrMap.put(0, QueryAttrEnum.CityTown);
	...
	@Action(value = "report", results = {
			@Result(name = INPUT, location = "cityOfNationList", type = "tiles"),
	...
	public String barChart_ajax() throws Exception
	{
		...
		attrMap.put(1, QueryAttrEnum.Nation);
		try
		{
